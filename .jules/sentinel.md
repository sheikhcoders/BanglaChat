...
## 2026-08-10 - Unsafe Inline Script Execution Vulnerabilities and CSP Mitigation

**Vulnerability:** XSS attacks and arbitrary client-side script execution from inline javascript elements.
**Learning:** Any inline <script> tag present in HTML makes it impossible to configure a strict Content Security Policy (CSP) that blocks inline script execution without using 'unsafe-inline'. This leaves the application highly susceptible to XSS if an attacker can inject HTML.
**Prevention:** Extract all inline scripts into separate, dedicated assets (such as app.js) and reference them via src. This allows the configuration of a highly restrictive Content Security Policy (CSP) using default-src 'self' and script-src 'self' to block all unauthorized inline and third-party script executions entirely.
