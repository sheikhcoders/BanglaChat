...
**Learning:** Hardening measures (like SHA pinning) must be balanced with "Bootstrap Guards" that allow CI to pass gracefully when no functional code is present. Insecure or brittle CI detection logic (like mandatory lockfile caching) can cause denial-of-service for repository updates.
**Prevention:** Use conditional steps (`if: hashFiles('...') != ''`) and robust fallback logic for package managers and artifacts to ensure CI availability across all repository lifecycle stages.
