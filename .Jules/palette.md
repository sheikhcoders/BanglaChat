...
## 2026-09-07 - Non-Intrusive Inline Button State Confirmation for Destructive Actions

**Learning:** Destructive actions (like clearing chat logs) can cause accidental data loss if executed on a single click. Modal dialogs introduce disruptive context switches for keyboard and screen reader users. Implementing an inline two-step button state confirmation ("⚠️ নিশ্চিত? / Confirm?") with dynamic ARIA label updates and an auto-reset timer provides clear safety guards while preserving user focus and interaction flow.
**Action:** Use inline 2-step button state toggles with clear ARIA label updates and auto-reset timers for destructive actions rather than intrusive modal overlays.
