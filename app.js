...
if (quickReplies) {
  const handleChipSelection = (chip) => {
    ...
    if (presetMsg) {
      ...
      announceStatus("বার্তা যোগ করা হয়েছে / Message added to input");
      input.focus();
    }
  };
...
input.addEventListener("input", () => {
  ...
  if (isAtLimit && currentInvalid !== "true") {
    ...
    announceStatus("সর্বোচ্চ ৫০০ অক্ষরের সীমা পৌঁছেছে / Maximum 500 character limit reached");
...
if (log) {
  log.addEventListener("click", (e) => {
    ...
    const notifyCopied = () => {
      ...
      announceStatus("বার্তা অনুলিপি করা হয়েছে / Message copied to clipboard");
      setTimeout(() => {
        btn.textContent = "📋";
        btn.setAttribute("aria-label", "বার্তা অনুলিপি করুন / Copy message");
        btn.setAttribute("title", "বার্তা অনুলিপি করুন / Copy message");
...
// Micro-UX Enhancement (Palette): Clear chat log action with inline 2-step confirmation & accessible empty state
if (clearLogBtn && log) {
  let clearConfirmTimer = null;

  const resetClearBtnState = () => {
    clearLogBtn.classList.remove("confirming");
    clearLogBtn.textContent = "🗑️ মুছুন / Clear";
    clearLogBtn.setAttribute("aria-label", "ইতিহাস মুছুন / Clear history");
    clearLogBtn.setAttribute("title", "ইতিহাস মুছুন / Clear history");
  ...
  clearLogBtn.addEventListener("click", () => {
    const hasUserMessages = Array.from(log.querySelectorAll("strong")).some(
      (el) => el.textContent.includes("আপনি"),
    );
    const isConfirming = clearLogBtn.classList.contains("confirming");

    if (hasUserMessages && !isConfirming) {
      clearLogBtn.classList.add("confirming");
      clearLogBtn.textContent = "⚠️ নিশ্চিত? / Confirm?";
      clearLogBtn.setAttribute(
        "aria-label",
        "ইতিহাস মোছা নিশ্চিত করুন / Confirm clearing history",
      );
      clearLogBtn.setAttribute(
        "title",
        "ইতিহাস মোছা নিশ্চিত করুন / Confirm clearing history",
    ...
    announceStatus("চ্যাট ইতিহাস খালি করা হয়েছে / Chat history cleared");
    input.focus();
  });
}
...
if (clearBtn) {
  clearBtn.addEventListener("click", () => {
    input.value = "";
    input.dispatchEvent(new Event("input", { bubbles: true }));
    announceStatus("লেখা মোছা হয়েছে / Text cleared");
    input.focus();
  });
}
...
// Micro-UX Enhancement (Palette): Document-wide '/' keyboard focus shortcut
document.addEventListener("keydown", (e) => {
  if (e.key === "/" && document.activeElement !== input) {
    e.preventDefault();
    input.focus();
    announceStatus("বার্তাঘরে ফোকাস করা হয়েছে / Focused message input");
  }
});
...
frm.addEventListener("submit", (e) => {
  ...
  announceStatus("বার্তা পাঠানো হয়েছে / Message sent");
