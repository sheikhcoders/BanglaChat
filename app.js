...
// This avoids repeated synchronous DOM tree queries on every submit or click action.
...
// Performance Optimization (Bolt): Use event delegation on `.quick-replies` container instead of attaching
// separate click event listeners to every individual chip button.
// Expected impact: Reduces memory allocations and event listener registrations from O(N) to O(1),
// eliminating duplicate handler callbacks and staying performant even if chips are updated dynamically.
if (quickReplies) {
...
input.addEventListener("input", () => {
  ...
  // Performance Optimization (Bolt): Guard className DOM mutations to prevent redundant style/layout recalculations.
  let newClass = "char-counter";
  if (len >= 500) {
    newClass = "char-counter danger";
  } else if (len >= 450) {
    newClass = "char-counter warning";
  }
  if (counter.className !== newClass) {
    counter.className = newClass;
  }
});
...
frm.addEventListener("submit", (e) => {
  ...
  // Performance Optimization (Bolt): Batch user message and typing indicator DOM insertions using a DocumentFragment.
  // Consolidates multiple DOM mutations into a single repaint and eliminates forced synchronous layout thrashing
  // by querying log.scrollHeight and setting log.scrollTop only once after batching.
